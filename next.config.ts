...
const withPWA = withPWAInit({
  dest: "public",
  register: false, // We'll register our own service worker
  skipWaiting: true,
  ...
  fallbacks: {
    document: '/offline', // Fallback to offline page for document (HTML) requests
    image: '/images/offline-image.svg', // Fallback image for failed image requests
    font: false, // Don't provide fallbacks for fonts
  },
  runtimeCaching: [
    {
...
const nextConfig: NextConfig = {
  ...
  headers: async () => {
    return [
      ...
      {
        source: "/workbox-error-handler.js",
        headers: [
          {
            key: "Cache-Control",
            value: "public, max-age=0, must-revalidate",
